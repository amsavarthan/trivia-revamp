...
fun loadEnergyRewardAd(context: Context, isTest: Boolean = BuildConfig.DEBUG) {
    if (isLoadingAd) return

    val unitId = if (isTest) {
        context.getString(R.string.ad_mob_test_reward_id)
    } else {
        context.getString(R.string.ad_mob_reward_id_for_energy)
    }
...
fun showEnergyRewardAd(
    context: Context,
    increaseEnergy: (Int) -> Unit,
    ...
    if (mRewardAd != null && activity != null) {
        ...
        mRewardAd?.show(activity) { reward ->
            val rewardAmount = reward.amount
            increaseEnergy(rewardAmount)
            rewardGranted = true
            Toast.makeText(
                context,
                "You earned $rewardAmount energy.",
                Toast.LENGTH_SHORT
            ).show()
        }
    } else {
